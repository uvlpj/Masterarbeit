...
class RandomForestWeight:
    ...
    def quantile_predict(self, q, X_test=None, w_all=None, top_k=None, batch_size=5000, verbose=False):
        ...
        y_sort = self.y_train.iloc[idx_sort]
        ...
        if isinstance(q, (list, pd.core.series.Series, np.ndarray)):
            ...
            for i, q_i in enumerate(q):
                ...
                print("Shape of y_sort.iloc[q_idxs]:", y_sort.iloc[q_idxs].shape)
                print("Shape of y_sort.iloc[q_idxs - 1]:", y_sort.iloc[q_idxs - 1].shape)
    ...
    def crps_score(y_true, y_pred, quantiles):
        """
        Berechnet den Continuous Ranked Probability Score (CRPS).
        Für den CRPS Score müssen Quantile Predictions gemacht werden
        Wahrscheinlichkeitsverteilung über Vorhersagen auf Basis von Quantilen verwenden
        Differenz zwischen den vorhergesagten kumulativen Verteilungen und der tatsächlichen Verteilung
    
        Parameters:
        - y_true: Tatsächliche Zielwerte (Testdaten).
        - y_pred: Vorhergesagte Quantile (array der Größe [n_samples, n_quantiles]).
        - quantiles: Die Quantile, für die die Vorhersage gemacht wurde (z.B. [0.01, 0.02, ..., 0.99]).
        ...
        for i in range(n_quantiles):
            diff = (y_pred[:, i] - y_true)
            crps += np.mean(diff * (quantiles[i] - (y_true < y_pred[:, i])))
        return crps / n_quantiles
